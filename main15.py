...
async def process_file_and_upload(c: Client, m: Message, in_path: Path, original_name: str = None, messages_to_delete: list = None, cancel_event_passed: asyncio.Event = None, passed_uid: int = None, default_caption: str = None):
    ...
    try:
        ...
        MAX_SPLIT_SIZE = 1.8 * 1024 * 1024 * 1024
        if is_video_file and upload_path.exists() and upload_path.stat().st_size > 1.9 * 1024 * 1024 * 1024 and duration_sec > 0:
            try:
                if status_msg:
                    await status_msg.edit("Video size is large, splitting video into smaller parts...", reply_markup=progress_keyboard())
            except Exception: pass
