...
async def multi_group_done_cb(c, cb):
    ...
    if uid in MULTI_GROUP_BATCH_MODE:
        ...
        try:
            await cb.message.edit_text(cb.message.text, reply_markup=None)
        except Exception: pass
        await cb.message.reply_text(f"New group created (Group {group_num}). Forward/send videos for this group.")
...
async def forwarded_file_or_direct_file(c: Client, m: Message):
    ...
    if uid in EDIT_CAPTION_MODE: 
        ...
        if uid in MULTI_GROUP_BATCH_MODE:
            ...
            await update_batch_status(c, m, uid, status_text, reply_markup=markup)
            return
        # -----------------------------------
        # --- NEW BATCH LOGIC ---
        if uid in BATCH_CAPTION_MODE:
    ...
    async with USER_UPLOAD_LOCKS[uid]:
        ...
        cmd = [
            ...
            *map_args,
            ...
            "-metadata:s", "title=[@TA_HD_Anime] Telegram Channel",
            "-metadata:s", "handler_name=[@TA_HD_Anime] Telegram Channel", 
            "-c", "copy",
            str(out_path)
        ]
