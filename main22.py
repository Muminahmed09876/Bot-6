...
CONVERT_WORKERS = {}
USER_CONVERT_LOCKS = {} # Queue lock for ffmpeg processing
# ----------------------------------
...
@app.on_callback_query(filters.regex("toggle_(audio|caption|ytdlp|zip|convert)_mode"))
...
async def execute_conversions(session_id, client):
    ...
    if uid not in USER_CONVERT_LOCKS:
        USER_CONVERT_LOCKS[uid] = asyncio.Lock()
        
    async with USER_CONVERT_LOCKS[uid]:
        try:
...
def build_bam_audio_keyboard(uid, tracks):
    state = BATCH_AUDIO_STATE[uid]
    primary = state.get('primary_track')
    includes = state.get('selected_includes', [])
    keyboard = []
    
    for i, track in enumerate(tracks, 1):
        t_title = track.get('title')
        if not t_title or t_title.lower() in ['n/a', 'unknown', '']:
            t_title = track.get('language', f"Track {i}")
            
        pri_text = "🟢 Pri" if primary == i else "⚪️ Pri"
        inc_text = "✅ Inc" if i in includes else "🔳 Inc"
        
        keyboard.append([
            InlineKeyboardButton(pri_text, callback_data=f"bam_aud_pri_{i}"),
            InlineKeyboardButton(t_title[:25], callback_data="ignore"),
            InlineKeyboardButton(inc_text, callback_data=f"bam_aud_inc_{i}")
        ])
        
    keyboard.append([InlineKeyboardButton("Select All & Auto Apply 🔄", callback_data="bam_aud_all")])
    keyboard.append([InlineKeyboardButton("Done ✅", callback_data="bam_aud_done")])
        
    return InlineKeyboardMarkup(keyboard)

@app.on_callback_query(filters.regex(r"^bam_aud_(pri|inc|all|done)"))
async def bam_audio_sel_cb(c, cb):
    ...
    if action == "pri":
        ...
        state['primary_track'] = track_idx
        if 'selected_includes' not in state: state['selected_includes'] = []
        if track_idx not in state['selected_includes']:
            state['selected_includes'].append(track_idx)
        await cb.answer(f"Track {track_idx} selected as primary.")
        await cb.message.edit_reply_markup(build_bam_audio_keyboard(uid, state['current_tracks']))
        
    elif action == "inc":
        track_idx = int(parts[3])
        if 'selected_includes' not in state: state['selected_includes'] = []
        if track_idx in state['selected_includes']:
            state['selected_includes'].remove(track_idx)
            if state.get('primary_track') == track_idx:
                state['primary_track'] = None
        else:
            state['selected_includes'].append(track_idx)
            if not state.get('primary_track'):
                state['primary_track'] = track_idx
        await cb.answer(f"Track {track_idx} inclusion toggled.")
        await cb.message.edit_reply_markup(build_bam_audio_keyboard(uid, state['current_tracks']))
        
    elif action == "all":
        if not state.get('selected_includes'):
            await cb.answer("Select at least one track to include!", show_alert=True)
            return
        if not state.get('primary_track'):
            state['primary_track'] = state['selected_includes'][0]
            
        current_tracks = state['current_tracks']
        sig = "-".join([f"{t['stream_index']}{t.get('language')}{t.get('title')}" for t in current_tracks])
        state['global_track_sig'] = sig
        state['global_primary'] = state['primary_track']
        state['global_includes'] = state['selected_includes'].copy()
        
        state['audio_ui_event'].set()
        await cb.answer("Configuration applied to this and upcoming matching files.")
        
    elif action == "done":
        if not state.get('selected_includes'):
            await cb.answer("Select at least one track!", show_alert=True)
            return
        if not state.get('primary_track'):
            state['primary_track'] = state['selected_includes'][0]
        
        state.pop('global_track_sig', None)
        
        state['audio_ui_event'].set()
        await cb.answer("Tracks selected.")
...
async def batch_audio_worker(uid, c):
    ...
    while not queue.empty():
        ...
        try:
            ...
            current_sig = "-".join([f"{t['stream_index']}{t.get('language')}{t.get('title')}" for t in tracks])
            
            if state.get('global_track_sig') == current_sig:
                primary_idx = state['global_primary']
                includes = state['global_includes']
            else:
                state['current_tracks'] = tracks
                state['primary_track'] = None
                state['selected_includes'] = []
                state['audio_ui_event'] = asyncio.Event()
                
                track_text = f"**Pair {idx}**\nBase: `{base_item['original_name']}`\nSrc: `{src_item['original_name']}`\n\n**Select Audio Track(s) from Source:**\n"
                for i, t in enumerate(tracks, 1):
                    track_text += f"{i}. Lang: {t.get('language', 'und')}, Title: {t.get('title', 'Unknown')}\n"
                    
                await status_msg.edit(track_text, reply_markup=build_bam_audio_keyboard(uid, tracks))
                await state['audio_ui_event'].wait()
                
                primary_idx = state.get('primary_track', 1)
                includes = state.get('selected_includes')
                if not includes:
                    includes = [1]
                    primary_idx = 1
            ...
            for i in includes:
                t = tracks[i - 1]
                src_stream_idx = t['stream_index']
                cmd.extend(["-map", f"1:{src_stream_idx}"])
                
                raw_title = t.get('title')
                if not raw_title or raw_title.lower() in ['n/a', 'unknown', '']:
                    raw_title = t.get('language', f"Track {i}")
                    
                title_counts[raw_title] = title_counts.get(raw_title, 0) + 1
                count = title_counts[raw_title]
                final_title = f"{raw_title}{count}" if count > 1 else raw_title
                
                disposition = "default" if i == primary_idx else "0"
                cmd.extend([f"-disposition:a:{out_audio_idx}", disposition])
                cmd.extend([f"-metadata:s:a:{out_audio_idx}", f"title={final_title}"])
