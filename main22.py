...
@app.on_callback_query(filters.regex("toggle_(audio|caption|ytdlp|zip|convert|batch_audio)_mode"))
    ...
    async with CONVERT_LOCKS[uid]: # Limit to one convert task at a time to save CPU
        ...
        try:
            for idx, config in enumerate(configs, 1):
                ...
                # Check Bitrate to set Preset
                if vb > orig_vb and orig_vb > 0:
                    # Ultrafast + bloat size forcing
                    cmd.extend(["-c:v", "libx264", "-preset", "ultrafast", "-threads", "0", 
                                "-b:v", str(vb), "-minrate", str(vb), "-maxrate", str(vb), "-bufsize", str(vb*2)])
                else:
                    # Veryfast/Medium for shrinking or default
...
async def text_handler(c, m: Message):
    ...
    # Handle zip convert mode
    if uid in CONVERT_MODE:
        if text_lower == "zip":
            CONVERT_ZIP_MODE.add(uid)
            await m.reply_text("Convert ZIP Mode **ON**. Send zip files, they will be extracted for conversion.")
            return
        elif text_lower == "zip off":
            CONVERT_ZIP_MODE.discard(uid)
            await m.reply_text("Convert ZIP Mode **OFF**. Normal URL/file processing active.")
            return
    ...
    if uid in BATCH_AUDIO_MODE:
        if text_lower == "next":
            ...
            # Parse custom mapping or use default
            pairs = []
            if text_lower == 'ok':
                ...
                try:
                    ...
                    for p in parts:
                        ...
                        if '=' in p:
                            ...
                            left_indices = []
                            if '-' in left:
                                s, e = map(int, left.split('-'))
                                left_indices = list(range(s-1, e))
                            else:
                                left_indices = [int(left)-1]
                            # Handle right side
                            right_indices = []
                            if '-' in right:
                                s, e = map(int, right.split('-'))
                                right_indices = list(range(s-1, e))
                            else:
                                right_indices = [int(r)-1 for r in right.split(',') if r.strip().isdigit()]
                                if not right_indices and right.isdigit():
                                    right_indices = [int(right)-1]
                                    
                            # Zip them together or broadcast single right to multiple left
    ...
    if text.startswith("http://") or text.startswith("https://"):
        url = text
        if uid in CONVERT_MODE:
            if uid in CONVERT_ZIP_MODE and ('zip' in url.lower() or 'rar' in url.lower() or '7z' in url.lower()):
                asyncio.create_task(execute_zip_download_and_extract(c, m, url=url, target_list=None))
                # Note: target_list is None so it falls back to check_and_show_next_zip, where user can see files. 
                # Better implementation: queue them to a convert list directly.
            else:
        ...
        if uid in BATCH_AUDIO_MODE:
            ...
            # Download and extract immediately to avoid blocking
            asyncio.create_task(process_batch_audio_input(uid, c, m, url=url, target_list=target_list))
            return
...
# --- BATCH AUDIO WORKER FOR LINKS/FILES ---
async def process_batch_audio_input(uid, c, m, url=None, file_info=None, target_list=None):
    ...
    if url and any(ext in url.lower() for ext in ['.zip', '.rar', '.7z']):
        await execute_zip_download_and_extract(c, m, url=url, target_list=target_list)
    elif file_info and file_info.file_name and any(file_info.file_name.lower().endswith(ext) for ext in ['.zip', '.rar', '.7z']):
        await execute_zip_download_and_extract(c, m, url=None, target_list=target_list)
    else:
        # Standard video
        try:
            status_msg = await m.reply_text("Downloading to build list...")
            original_name = "video.mp4"
            if url: original_name = await get_filename_from_url(url)
            elif file_info: original_name = file_info.file_name or "video.mp4"
            
            safe_name = re.sub(r"[\\/*?\"<>|:]", "_", original_name)
            tmp_in = TMP / f"ba_{uid}_{int(time.time())}_{safe_name}"
            
            if url:
                if is_drive_url(url):
                    fid = extract_drive_id(url)
                    if fid: await download_drive_file(fid, tmp_in, status_msg)
                else:
                    await download_url_generic(url, tmp_in, status_msg)
            elif file_info:
...
async def forwarded_file_or_direct_file(c: Client, m: Message):
    ...
    if uid in CONVERT_MODE:
        if uid in CONVERT_ZIP_MODE and any(original_name.lower().endswith(ext) for ext in ['.zip', '.rar', '.7z']):
            asyncio.create_task(execute_zip_download_and_extract(c, m, url=None, target_list=None))
        else:
    ...
    if uid in BATCH_AUDIO_MODE:
        state = BATCH_AUDIO_STATE.get(uid)
        target_list = BATCH_AUDIO_LIST1[uid] if state == 'list1' else BATCH_AUDIO_LIST2[uid]
        asyncio.create_task(process_batch_audio_input(uid, c, m, file_info=file_info, target_list=target_list))
        return
...
async def execute_batch_audio_remux(uid, c, chat_id):
    ...
    for i, (idx1, idx2) in enumerate(pairs):
        ...
        out_name = generate_new_filename(base_name)
        ...
        if def_track != -1 and def_track in keep_tracks:
            # find its new index in the mapped audio streams
    ...
    await status_msg.edit("Batch Audio Processing Complete!")
