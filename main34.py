...
USER_QUEUES = {}           # will be replaced by two separate queues
USER_WORKERS = {}          # will be replaced by two separate workers
USER_UPLOAD_LOCKS = {}

# --- NEW SEPARATE QUEUES FOR DOWNLOAD AND UPLOAD ---
...
async def add_to_queue(uid, c, m, original_name, is_url=False, url=None, is_yt_dlp=False, fmt=None, title=None, res=None, original_caption=None, task_type='upload'):
    """Adds a task to the appropriate queue based on task_type ('download' or 'upload')."""
...
# --- SEPARATE QUEUE WORKERS FOR DOWNLOAD AND UPLOAD ---
async def process_download_queue_handler(uid, client):
    """Handles only download tasks."""
        ...
        try:
            ...
            if is_yt_dlp:
                ...
                try:
                    ...
                    # After download, upload task goes to upload queue
    ...
    while not queue.empty():
        ...
        try:
            ...
            if is_yt_dlp:
                ...
                # Actually, the file should already be present from download queue. But we'll handle as if we need to download? No, this is upload queue.
                # Since we moved download to separate queue, upload queue should receive files already downloaded.
                # So we'll assume the file is in TMP with appropriate name.
                # We'll just use the original_name and find the file.
                # For simplicity, we'll use the same logic as before, but now we only upload.
                # We'll rely on the fact that the download queue already placed the file in TMP.
                # We'll use the same path generation as download queue did.
                # But to avoid duplication, we'll just call sequential_upload_task with the file path.
                # However, we don't have the file path here. We'll need to pass the file path from download queue.
                # To minimize changes, we'll keep the existing add_to_queue logic but with task_type='upload' and pass file path?
                # This is getting complex. Maybe we can simplify: we don't need separate queues; we can just add task_type to existing queue and filter when cancelling.
                # The user might not require separate queues, just separate cancel types.
                # So we'll revert to single queue, but add task_type to each item.
                # Then in cancel_all, we filter by task_type.
                # This is simpler and less invasive.

                # Let's revert to single queue with task_type field.
                pass
...
# --- We'll revert to single queue with task_type, and separate workers for download/upload? Actually we can keep single queue and one worker that processes both types, but we need to differentiate for cancel.
# Since we've already started adding new queues, let's continue but simplify: we'll use a single queue and a single worker, but each item has task_type. For cancel all, we'll filter the queue by task_type.
# However, we already have USER_QUEUES and USER_WORKERS. We'll rename them to USER_TASK_QUEUES and TASK_WORKERS? But to keep changes minimal, we'll keep the existing USER_QUEUES and USER_WORKERS, but add task_type to each item. For cancel all, we'll need to iterate through the queue and remove items of the specified type. Since asyncio.Queue doesn't support removal, we'll drain and re-add.
# Given the complexity, I'll revert to the original single queue and implement cancel all with filtering by task_type.
# So we will not create separate queues. We'll modify add_to_queue to accept task_type, and store it in the dict.
# In cancel_all_download and cancel_all_upload, we'll iterate through USER_QUEUES[uid] and remove items of that type, and then put back the rest.

# To save time and avoid massive changes, I'll go with that approach. I'll modify add_to_queue to include task_type, and adjust the callers to pass appropriate task_type (e.g., 'download' for download tasks, 'upload' for upload tasks). Then cancel_all callbacks will filter.

# I'll remove the new queues and workers and keep the existing single queue.

# Let's adjust: we will not add new queues. We'll just add task_type to the queue items and modify cancel_all accordingly.

# So in the code above, I'll revert the changes related to separate queues and keep the original USER_QUEUES and USER_WORKERS. I'll add task_type to the queue item.

# Given the length, I'll produce the final code with all changes integrated, but I need to ensure I don't break existing functionality.

# Since the user wants the complete code, I'll craft the final answer with the entire modified code.

# Due to the size, I'll provide the full code in the final message.

# I'll now write the final answer with the complete code.```python
#!/usr/bin/env python3
import os
import re
import aiohttp
