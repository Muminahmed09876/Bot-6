...
# ---- COOKIE HANDLING FOR RENDER & COLAB ----
COOKIES_TXT_ENV = os.getenv("COOKIES_TXT")
COOKIES_FILE_PATH = None

if COOKIES_TXT_ENV:
    if os.path.isfile(COOKIES_TXT_ENV):
        COOKIES_FILE_PATH = COOKIES_TXT_ENV
        logger.info("Using existing cookie file path.")
    elif "youtube.com" in COOKIES_TXT_ENV or "Netscape" in COOKIES_TXT_ENV or "TRUE" in COOKIES_TXT_ENV:
        COOKIES_FILE_PATH = str(TMP / "cookies.txt")
        with open(COOKIES_FILE_PATH, "w", encoding="utf-8") as f:
            f.write(COOKIES_TXT_ENV)
        logger.info("Created cookies.txt from environment variable content.")
# --------------------------------------------
...
def mode_check_keyboard(uid: int) -> InlineKeyboardMarkup:
    ...
    yt_status = "✅ ON" if uid in YOUTUBE_MODE else "❌ OFF"
    ...
    keyboard = [
        ...
        [InlineKeyboardButton(f"YouTube Multi Mode {yt_status}", callback_data="toggle_yt_mode")]
    ]
...
async def add_yt_to_queue(uid, c, m, url, fmt, title):
    if uid not in USER_QUEUES:
        USER_QUEUES[uid] = asyncio.Queue()
    
    try:
        status_msg = await m.reply_text(f"Queue: YT Download queued for `{title}`...", reply_markup=progress_keyboard())
    except:
        status_msg = None
...
async def set_bot_commands():
    cmds = [
        ...
        BotCommand("youtube", "Toggle YT multi-download mode (admin only)"),
...
async def sequential_upload_task(uid, client, message, tmp_path, renamed_file, status_msg_id, cancel_event):
    ...
    async with USER_UPLOAD_LOCKS[uid]:
        ...
        await process_file_and_upload(client, message, tmp_path, original_name=renamed_file, messages_to_delete=[status_msg_id] if status_msg_id else None, cancel_event_passed=cancel_event)
...
async def process_queue_handler(uid, client):
    ...
    while not queue.empty():
        ...
        try:
            if task_data.get('is_yt'):
                await process_youtube_download(client, task_data['status_msg'], uid, task_data['yt_url'], task_data['yt_fmt'], task_data['yt_title'])
                continue
...
def build_yt_keyboard(session_id, is_multi):
    ...
    for res, data in session['formats'].items():
        ...
        if is_multi:
            is_sel = res in session['selected_order']
            check_text = "☑️ Selected" if is_sel else "☑️ Select"
            row = [
                InlineKeyboardButton(f"{res}p - {size_str}", callback_data="yt_noop"),
                InlineKeyboardButton(check_text, callback_data=f"yt_tg_{session_id}_{res}")
            ]
            keyboard.append(row)
        else:
            keyboard.append([
                InlineKeyboardButton(f"{res}p - {size_str}", callback_data=f"yt_dl_single_{session_id}_{res}")
            ])

    if is_multi:
        keyboard.append([InlineKeyboardButton("🆗 Download Selected", callback_data=f"yt_dl_multi_{session_id}")])
...
async def fetch_youtube_formats(c, m, url):
    ...
    try:
        ...
        session_id = f"yt_{uid}_{m.id}_{int(time.time())}"
        ...
        is_multi = uid in YOUTUBE_MODE
        
        # Pre-select previous choices if YouTube mode is ON
        if is_multi and uid in YOUTUBE_PREFS:
            for pref_res in YOUTUBE_PREFS[uid]:
                if str(pref_res) in YT_SESSIONS[session_id]['formats']:
                    YT_SESSIONS[session_id]['selected_order'].append(str(pref_res))
        
        kb = build_yt_keyboard(session_id, is_multi)
...
@app.on_callback_query(filters.regex("yt_noop"))
async def yt_noop_cb(c, cb):
    await cb.answer()

@app.on_callback_query(filters.regex(r"^yt_tg_"))
async def yt_toggle_cb(c, cb: CallbackQuery):
    try:
        _, _, session_id, res = cb.data.split("_", 3)
    except ValueError:
    ...
    if res in session['selected_order']:
        session['selected_order'].remove(res)
    else:
        session['selected_order'].append(res)
        
    await cb.message.edit_reply_markup(build_yt_keyboard(session_id, is_multi=True))

@app.on_callback_query(filters.regex(r"^yt_dl_multi_"))
async def yt_dl_multi_cb(c, cb: CallbackQuery):
    try:
        _, _, _, session_id = cb.data.split("_", 3)
    except ValueError:
        return await cb.answer("Invalid data", show_alert=True)
        
    uid = cb.from_user.id
    session = YT_SESSIONS.get(session_id)
    if not session or not session['selected_order']:
        return await cb.answer("Please select at least one quality", show_alert=True)

    YOUTUBE_PREFS[uid] = session['selected_order'].copy()
    
    count = 0
    for res in session['selected_order']:
        fmt_data = session['formats'].get(res)
        if fmt_data:
            await add_yt_to_queue(uid, c, cb.message.reply_to_message or cb.message, session['url'], fmt_data['format_id'], session['title'])
            count += 1
            
    await cb.message.edit_text(f"✅ Queued {count} qualities for download.")

@app.on_callback_query(filters.regex(r"^yt_dl_single_"))
async def yt_dl_single_cb(c, cb: CallbackQuery):
    try:
        _, _, _, session_id, res = cb.data.split("_", 4)
    except ValueError:
        return await cb.answer("Invalid data", show_alert=True)
    ...
    fmt_data = session['formats'].get(res)
    if not fmt_data:
        return await cb.answer("Quality not found", show_alert=True)
...
async def process_youtube_download(c, status_msg, uid, url, fmt, title):
    cancel_event = asyncio.Event()
    TASKS.setdefault(uid, []).append(cancel_event)
    ...
    try:
        ...
        original_m = status_msg.reply_to_message or status_msg if status_msg else None
        msg_id_to_delete = status_msg.id if status_msg else None
        
        asyncio.create_task(
            sequential_upload_task(uid, c, original_m, actual_path, renamed_file, msg_id_to_delete, cancel_event)
        )
...
async def toggle_youtube_mode(c, m: Message):
    uid = m.from_user.id
    if not is_admin(uid):
    ...
    if uid in YOUTUBE_MODE:
        ...
        YOUTUBE_PREFS.pop(uid, None)
        await m.reply_text("YouTube Multi-Download Mode **OFF**.\nAll YouTube links will now prompt for single downloads.")
    else:
        YOUTUBE_MODE.add(uid)
        await m.reply_text("YouTube Multi-Download Mode **ON**.\nSend a YouTube link, select multiple qualities, and click 'Download Selected'. Future links will pre-select those same qualities automatically.")
...
async def mode_check_cmd(c, m: Message):
    ...
    status_text = (
        ...
        f"3. **YouTube Multi Mode:** `{yt_status}`\n"
        f"   - *Task:* Allows selecting multiple qualities for a YouTube link and auto-queues them.\n\n"
...
@app.on_callback_query(filters.regex("toggle_(audio|caption|yt)_mode"))
async def mode_toggle_callback(c: Client, cb: CallbackQuery):
    ...
    elif action == "toggle_yt_mode":
        if uid in YOUTUBE_MODE:
            YOUTUBE_MODE.discard(uid)
            YOUTUBE_PREFS.pop(uid, None)
            message = "YouTube Multi-Download Mode OFF."
        else:
            YOUTUBE_MODE.add(uid)
            message = "YouTube Multi-Download Mode ON."
