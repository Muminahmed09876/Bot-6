...
async def set_bot_commands():
    cmds = [
        ...
        BotCommand("yt_dlp", "Toggle YT-DLP mode for all links (admin only)"),
...
async def sequential_upload_task(uid, client, message, tmp_path, renamed_file, status_msg_id, cancel_event, fallback_caption=None):
    ...
    async with USER_UPLOAD_LOCKS[uid]:
        ...
        await process_file_and_upload(client, message, tmp_path, original_name=renamed_file, messages_to_delete=[status_msg_id] if status_msg_id else [], cancel_event_passed=cancel_event, fallback_caption=fallback_caption)
    ...
    while not queue.empty():
        ...
        try:
            if task_data.get('is_yt_dlp'):
                ...
                formats = task_data['formats_to_dl']

                for fmt in formats:
                    cancel_event = asyncio.Event()
                    TASKS.setdefault(uid, []).append(cancel_event)

                    dl_format = fmt['dl_format']
                    res = fmt['res']
                    
                    if status_msg:
                        try:
                            await status_msg.edit(f"Downloading {res}p...", reply_markup=progress_keyboard())
                        except: pass

                    try:
                        ydl_opts = {
                            'format': dl_format,
                            'outtmpl': str(TMP / f"yt_{uid}_{int(datetime.now().timestamp())}_%(title).100s.%(ext)s"),
                            'quiet': True,
                            'no_warnings': True,
                        ...
                        fallback_caption = f"**{title} - {res}p**"
                        
                        # Sequential upload phase inside the worker loop
                        asyncio.create_task(
                            sequential_upload_task(uid, client, task_data['message'], actual_path, renamed_file, status_msg.id if status_msg else None, cancel_event, fallback_caption=fallback_caption)
                        )

                    except Exception as e:
                        logger.error(f"YT-DLP Queue Error: {e}")
                        try:
                            if status_msg:
                                await status_msg.edit_text(f"YouTube Download Error: {e}")
                        except Exception: pass
                    finally:
                        # Event will be cleaned up inside upload task
                        pass
...
# --- YT-DLP UI HELPER ---
def build_yt_keyboard(session_id: str) -> InlineKeyboardMarkup:
    session = YT_SESSIONS.get(session_id)
    if not session:
        return InlineKeyboardMarkup([])
        
    keyboard = []
    for i, f in enumerate(session['formats']):
        res = f['res']
        size = f['size_str']
        sel_text = ""
        if i in session['selected']:
            order = session['selected'].index(i) + 1
            sel_text = f" ✅ ({order})"
            
        btn_dl = InlineKeyboardButton(f"{res}p - {size}", callback_data=f"yt_dl_{session_id}_{i}")
        btn_sel = InlineKeyboardButton(f"Select{sel_text}", callback_data=f"yt_sel_{session_id}_{i}")
...
async def queue_yt_downloads(c, uid, session, formats_to_dl, status_msg):
    task_data = {
        'is_yt_dlp': True,
        'message': status_msg,
        'url': session['url'],
        'title': session['title'],
        'formats_to_dl': formats_to_dl,
        'status_msg': status_msg,
...
async def fetch_youtube_formats(c, m, url):
    ...
    try:
        ...
        count = len(YT_SESSIONS)
        session_id = f"{uid}_{count}"
        ...
        for f in formats:
            ...
            if vcodec != 'none' and res and size:
                if res not in added_resolutions:
                    ...
                    YT_SESSIONS[session_id]['formats'].append({
                        'res': res,
                        'size_str': size_str,
                        'dl_format': dl_format,
                        'ext': ext
...
@app.on_callback_query(filters.regex(r"^yt_(dl|sel|ok|save|load)_(\d+)_(\d+)(?:_(\d+))?$"))
async def yt_multi_handler(c: Client, cb: CallbackQuery):
    action = cb.matches[0].group(1)
    uid_str = cb.matches[0].group(2)
    count_str = cb.matches[0].group(3)
    idx_str = cb.matches[0].group(4)
    ...
    if action == "sel":
        ...
        idx = int(idx_str)
        formats_to_dl = [ session['formats'][idx] ]
        await queue_yt_downloads(c, uid, session, formats_to_dl, cb.message)
        await cb.answer("Added to download queue!")

    elif action == "ok":
        if not session['selected']:
            await cb.answer("Please select at least one quality first.", show_alert=True)
            return
        formats_to_dl = [session['formats'][i] for i in session['selected']]
        await queue_yt_downloads(c, uid, session, formats_to_dl, cb.message)
        await cb.answer(f"Added {len(formats_to_dl)} qualities to download queue!")

    elif action == "save":
        if not session['selected']:
            await cb.answer("Please select at least one quality to save.", show_alert=True)
            return
        saved_res = [session['formats'][i]['res'] for i in session['selected']]
        SAVED_YT_QUALITIES[uid] = saved_res
        await cb.answer("Qualities saved successfully!", show_alert=True)

    elif action == "load":
        saved_res = SAVED_YT_QUALITIES.get(uid, [])
        if not saved_res:
            await cb.answer("No saved qualities found.", show_alert=True)
            return
        for res in saved_res:
            for i, fmt in enumerate(session['formats']):
                if fmt['res'] == res and i not in session['selected']:
                    session['selected'].append(i)
                    break
        await cb.message.edit_reply_markup(build_yt_keyboard(session_id))
        await cb.answer("Saved qualities loaded.")
...
async def start_handler(c, m: Message):
    ...
    text = (
        ...
        "/yt_dlp - Toggle YT-DLP download mode for all links (admin only)\n"
...
@app.on_message(filters.command(["yt_dlp", "yt-dlp"]) & filters.private)
async def toggle_ytdlp_mode(c, m: Message):
    uid = m.from_user.id
    if not is_admin(uid):
    ...
    if uid in YT_DLP_MODE:
        ...
        await m.reply_text("YT-DLP mode has been **TURNED OFF**. Links will be downloaded directly.")
    else:
        YT_DLP_MODE.add(uid)
        await m.reply_text("YT-DLP mode has been **TURNED ON**. Links provided to the bot will be processed via yt-dlp.")
...
async def mode_check_cmd(c, m: Message):
    ...
    status_text = (
        ...
        f"   - *Task:* Processes all submitted URLs via yt-dlp first.\n\n"
...
async def process_file_and_upload(c: Client, m: Message, in_path: Path, original_name: str = None, messages_to_delete: list = None, cancel_event_passed: asyncio.Event = None, fallback_caption: str = None):
    ...
    try:
        ...
        SPLIT_LIMIT = int(1.8 * 1024 * 1024 * 1024)
