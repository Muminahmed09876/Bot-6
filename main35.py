...
# ---- global state for multiple bots ----
BOT_INSTANCES = {}  # key: bot_id (str), value: {'client': Client, 'tmp': Path, 'start_time': datetime, 'name': str}
MAIN_BOT_ID = "main"
# -----------------------------------------

# state (shared among all bots, but each bot has its own tmp)
USER_THUMBS = {}
TASKS = {}
...
NAV_PATHS = {} # uid -> {"current": Path, "items": list, "selected_file": Path, "page": int, "msg_id": int}
# ------------------------------------------------
...
# --- NEW STATE FOR GOOGLE DRIVE UPLOAD MODE ---
...
async def download_drive_file(file_id: str, out_path: Path, message: Message = None, cancel_event: asyncio.Event = None, original_name=None):
    # (Issue 4) - Use Service Account if available for GDrive downloads
    service = get_gdrive_service()
    if service:
        try:
                ...
                with out_path.open(mode) as fh:
                    ...
                    while done is False:
                        ...
                        if status and message:
                            # Safely call async progress from sync thread
...
# ---- bot management commands (only on main bot) ----
async def bot_command_handler(c: Client, m: Message):
    uid = m.from_user.id
    if not is_admin(uid):
        return
    text = m.text.strip()
    ...
    if len(parts) < 2:
        await m.reply_text("Usage: `bot <token>` to start a new bot, or `bot list` to list running bots.")
        return
    subcmd = parts[1].strip()

    if subcmd == "list":
        if not BOT_INSTANCES:
            await m.reply_text("No additional bots are running.")
            return
        keyboard = []
        for bot_id, info in BOT_INSTANCES.items():
            if bot_id == MAIN_BOT_ID:
                continue
            name = info.get('name', bot_id[:8])
            keyboard.append([InlineKeyboardButton(f"{name} (since {info['start_time'].strftime('%H:%M')})", callback_data=f"bot_info_{bot_id}")])
            keyboard.append([InlineKeyboardButton(f"🗑 Delete {name}", callback_data=f"bot_stop_{bot_id}")])
        if not keyboard:
            await m.reply_text("No additional bots running.")
            return
        await m.reply_text("**Running bots:**", reply_markup=InlineKeyboardMarkup(keyboard))
        return
    ...
    for bot_id, info in BOT_INSTANCES.items():
        if info.get('token') == token:
            await m.reply_text(f"Bot with token `{token[:8]}...` is already running as `{info.get('name', bot_id)}`.")
            return
    ...
    # we need to use a separate Client instance
    new_client = Client(
        bot_id,
        api_id=API_ID,
        api_hash=API_HASH,
    ...
    # setup handlers for this client
    setup_handlers(new_client)
    # start the client
    try:
        await new_client.start()
    except Exception as e:
        await m.reply_text(f"Failed to start bot with token `{token[:8]}...`: {e}")
        return
    ...
    BOT_INSTANCES[bot_id] = {
        ...
        'name': f"Bot {len(BOT_INSTANCES)+1}",
        'token': token
    }
    await m.reply_text(f"✅ New bot started successfully!\nName: {BOT_INSTANCES[bot_id]['name']}\nToken: `{token[:8]}...`\nTmp: `{tmp_dir}`")

# callback for bot info and stop
async def bot_callback_handler(c: Client, cb: CallbackQuery):
    uid = cb.from_user.id
    if not is_admin(uid):
    ...
    if data.startswith("bot_stop_"):
        ...
        if bot_id not in BOT_INSTANCES:
            await cb.answer("Bot not found.")
            return
        info = BOT_INSTANCES[bot_id]
        client = info['client']
        try:
            await client.stop()
        except Exception as e:
            await cb.answer(f"Error stopping bot: {e}")
            return
        # clean tmp
        try:
            shutil.rmtree(info['tmp'])
        except:
            pass
        del BOT_INSTANCES[bot_id]
        await cb.answer(f"Bot {info.get('name', bot_id)} stopped.", show_alert=True)
        ...
        if bot_id not in BOT_INSTANCES:
            ...
            return
        info = BOT_INSTANCES[bot_id]
        await cb.answer(f"Bot: {info.get('name', bot_id)}\nStarted: {info['start_time']}\nTmp: {info['tmp']}", show_alert=True)

# ----------------------------------------------------

# ===== Setup handlers for any client =====
def setup_handlers(client):
    # Add all message handlers
    client.add_handler(MessageHandler(start_handler, filters.command("start") & filters.private))
    client.add_handler(MessageHandler(help_handler, filters.command("help") & filters.private))
    client.add_handler(MessageHandler(continue_cmd, filters.command("continue") & filters.private))
    client.add_handler(MessageHandler(restart_cmd, filters.command("restart") & filters.private))
    client.add_handler(MessageHandler(toggle_upload_drive_mode, filters.command("upload_drive") & filters.private))
    client.add_handler(MessageHandler(zip_file_download_cmd, filters.command("zip_file_download") & filters.private))
    client.add_handler(MessageHandler(toggle_yt_dlp, filters.command("yt_dlp") & filters.private))
    client.add_handler(MessageHandler(toggle_convert_mode, filters.command("convert") & filters.private))
    client.add_handler(MessageHandler(toggle_download_only_mode, filters.command("download_only") & filters.private))
    client.add_handler(MessageHandler(progress_bar_cmd, filters.command("progress_bar") & filters.private))
    client.add_handler(MessageHandler(setthumb_prompt, filters.command("setthumb") & filters.private))
    client.add_handler(MessageHandler(view_thumb_cmd, filters.command("view_thumb") & filters.private))
    client.add_handler(MessageHandler(del_thumb_cmd, filters.command("del_thumb") & filters.private))
    client.add_handler(MessageHandler(set_caption_prompt, filters.command("set_caption") & filters.private))
    client.add_handler(MessageHandler(view_caption_cmd, filters.command("view_caption") & filters.private))
    client.add_handler(MessageHandler(set_filename_prompt, filters.command("file_name_save") & filters.private))
    client.add_handler(MessageHandler(view_filename_cmd, filters.command("view_filename") & filters.private))
    client.add_handler(MessageHandler(toggle_edit_caption_mode, filters.command("edit_caption_mode") & filters.private))
    client.add_handler(MessageHandler(batch_audio_add_mode, filters.command("batch_audio_add") & filters.private))
    client.add_handler(MessageHandler(toggle_audio_change_mode, filters.command("mkv_video_audio_change") & filters.private))
    client.add_handler(MessageHandler(toggle_create_post_mode, filters.command("create_post") & filters.private))
    client.add_handler(MessageHandler(mode_check_cmd, filters.command("mode_check") & filters.private))
    client.add_handler(MessageHandler(check_drive_cmd, filters.command("check_drive") & filters.private))
    client.add_handler(MessageHandler(rename_cmd, filters.command("rename") & filters.private))
    client.add_handler(MessageHandler(upload_url_cmd, filters.command("upload_url") & filters.private))
    client.add_handler(MessageHandler(check_cmd, filters.command("check") & filters.private))
    # bot command (only for main bot, but we add it anyway)
    client.add_handler(MessageHandler(bot_command_handler, filters.command("bot") & filters.private))

    # photo handler
    client.add_handler(MessageHandler(photo_handler, filters.photo & filters.private))

    # text handler (catch-all)
    client.add_handler(MessageHandler(text_handler, filters.text & filters.private))

    # video and document handler
    client.add_handler(MessageHandler(forwarded_file_or_direct_file, filters.private & (filters.video | filters.document)))

    # ---- callback query handlers ----
    client.add_handler(CallbackQueryHandler(ext_select_cb, filters.regex(r"^extsel_")))
    client.add_handler(CallbackQueryHandler(refresh_btn_cb, filters.regex("refresh_btn")))
    client.add_handler(CallbackQueryHandler(queue_continue_cb, filters.regex("queue_continue")))
    client.add_handler(CallbackQueryHandler(queue_delete_cb, filters.regex("queue_delete")))
    client.add_handler(CallbackQueryHandler(yt_multi_callback, filters.regex(r"^yt(dir|sel|ok|save|load)_")))
    client.add_handler(CallbackQueryHandler(cancel_yt_cb, filters.regex("cancel_yt")))
    client.add_handler(CallbackQueryHandler(multi_group_done_cb, filters.regex("multi_group_done")))
    client.add_handler(CallbackQueryHandler(mode_toggle_callback, filters.regex("toggle_(audio|caption|ytdlp|zip|convert|batch_audio|dl_only|drive)_mode")))
    client.add_handler(CallbackQueryHandler(zip_upload_all_cb, filters.regex("zip_upload_all")))
    client.add_handler(CallbackQueryHandler(zip_cancel_cb, filters.regex("zip_cancel")))
    client.add_handler(CallbackQueryHandler(path_page_cb, filters.regex(r"^path_page_")))
    client.add_handler(CallbackQueryHandler(path_back_cb, filters.regex(r"^path_back_")))
    client.add_handler(CallbackQueryHandler(path_mode_cb, filters.regex(r"^path_mode_")))
    client.add_handler(CallbackQueryHandler(path_nextlist_cb, filters.regex(r"^path_nextlist_")))
    client.add_handler(CallbackQueryHandler(path_all_cb, filters.regex(r"^path_all_")))
    client.add_handler(CallbackQueryHandler(path_done_cb, filters.regex(r"^path_done_")))
    client.add_handler(CallbackQueryHandler(path_close_cb, filters.regex(r"^path_close_")))
    client.add_handler(CallbackQueryHandler(convert_cb_handler, filters.regex(r"^cv_(res|vb_minus|vb_plus|ab_minus|ab_plus|orig|add|next|selectall)_")))
    client.add_handler(CallbackQueryHandler(batch_audio_callback, filters.regex(r"^baud_")))
    client.add_handler(CallbackQueryHandler(cancel_single_cb, filters.regex("cancel_single")))
    client.add_handler(CallbackQueryHandler(cancel_all_download_cb, filters.regex("cancel_all_download")))
    client.add_handler(CallbackQueryHandler(cancel_all_upload_cb, filters.regex("cancel_all_upload")))
    client.add_handler(CallbackQueryHandler(cancel_all_cb, filters.regex("cancel_all")))  # legacy
    client.add_handler(CallbackQueryHandler(delete_caption_cb, filters.regex("delete_caption")))
    client.add_handler(CallbackQueryHandler(delete_filename_cb, filters.regex("delete_filename")))
    client.add_handler(CallbackQueryHandler(clear_tmp_data_cb, filters.regex("clear_tmp_data")))
    client.add_handler(CallbackQueryHandler(full_reset_bot_cb, filters.regex("full_reset_bot")))
    client.add_handler(CallbackQueryHandler(bot_callback_handler, filters.regex(r"^bot_(stop|info)_")))

    # Set bot commands
    asyncio.create_task(set_bot_commands(client))

# ===== Handlers definitions (unchanged, but now without decorators) =====
async def start_handler(c, m: Message):
    await set_bot_commands(c)
    text = (
...
async def photo_handler(c, m: Message):
    ...
    if uid in SET_THUMB_REQUEST:
        ...
        # Issue 4: Support photo direct upload to drive
        if uid in UPLOAD_DRIVE_MODE:
...
async def path_all_cb(c, cb):
    ...
    if mode == 'convert':
        ...
        # Delete the path UI message after starting upload
...
async def path_done_cb(c, cb):
    ...
    if mode == 'convert':
        ...
        # Auto-delete status after 5 seconds? User wants some messages to stay, but this one is okay to delete? They said "Starting upload of 6 files across 1 lists... এই ধরনের মেসেজ থাকবে এগুলো কে delete করবে না" – they want to keep this message. So we won't delete it.
...
async def convert_cb_handler(c: Client, cb: CallbackQuery):
    ...
    if action == "res":
        ...
        # Save current config to configs list, reset current to defaults
        session['configs'].append(session['current_config'].copy())
        # Reset current to original metadata
        meta = session['meta']
        ...
        if batch_list and session['batch_index'] + 1 < len(batch_list):
            ...
            # No more videos, start conversion
        ...
        # Save current config
        session['configs'].append(session['current_config'].copy())
        # Apply this configs list to all videos in batch
        batch_list = session.get('batch_list')
        if batch_list:
            # For each other video, set its configs to the same list (shallow copy is fine)
            for idx, item in enumerate(batch_list):
                if idx != session['batch_index']:
                    # We'll store configs in a separate structure, but for simplicity we'll just store them in the session's batch list
                    item['configs'] = session['configs'].copy()
                    item['upload_original'] = session['upload_original']
            # Also store for current video
            ...
            # Single video case: just use current configs
            pass
...
async def execute_conversions(session_id, client, batch_apply=False):
    ...
    if batch_apply and session.get('batch_list'):
        # Use batch_list with configs stored per video
        for item in session['batch_list']:
            f_path = Path(item['path'])
            if f_path.exists():
                configs = item.get('configs', [])
                if not configs:
                    # If no configs, maybe use the session's configs? Fallback to empty.
                    configs = []
            ...
            if files_to_process:
                if is_delete:
                    ...
                    if mode:
                        ...
                        # Delete path UI after starting upload
                        if uid in NAV_PATHS and 'msg_id' in NAV_PATHS[uid]:
                            try:
                                await c.delete_messages(m.chat.id, NAV_PATHS[uid]['msg_id'])
                            except:
...
if __name__ == "__main__":
    ...
    # setup handlers for main bot
    setup_handlers(main_client)
    # store main bot in instances
    BOT_INSTANCES[MAIN_BOT_ID] = {
        'client': main_client,
        'tmp': TMP,
        'start_time': datetime.now(),
        'name': 'Main Bot',
