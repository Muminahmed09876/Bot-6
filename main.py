...
# --- NEW STATE FOR MULTI GROUP BATCH ---
...
async def sequential_upload_task(uid, client, message, tmp_path, renamed_file, status_msg_id, cancel_event, default_caption=None):
    ...
    async with USER_UPLOAD_LOCKS[uid]:
        ...
        await process_file_and_upload(client, message, tmp_path, original_name=renamed_file, messages_to_delete=[status_msg_id], cancel_event_passed=cancel_event, passed_uid=uid, default_caption=default_caption)
...
async def text_handler(c, m: Message):
    ...
    if text_lower in ["on", "off", "ok", "no", "d"]:
        if uid in EDIT_CAPTION_MODE:
            if text_lower == "on":
