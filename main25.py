    ...
    async with CONVERT_LOCKS[uid]: # Limit to one convert task at a time to save CPU
        ...
        try:
            for idx, config in enumerate(configs, 1):
                ...
                # Check Bitrate to set Preset (Medium vs Ultrafast)
                if vb > orig_vb and orig_vb > 0:
                    ...
                    # Default: Medium preset for shrinking or standard encode
                    cmd.extend(["-c:v", "libx264", "-preset", "medium", "-threads", "0", "-b:v", str(vb)])
