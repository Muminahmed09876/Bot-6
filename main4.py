        ...
        for f in formats:
            ...
            if vcodec != 'none' and res and size:
                if res not in added_resolutions:
                    ...
                    cb_data = f"yt_dl_{uid}_{len(YT_SESSIONS)}"
                    YT_SESSIONS[cb_data] = {
                        'url': url,
                        'format': dl_format,
                        'res': res,
                    ...
                    keyboard.append([InlineKeyboardButton(f"{res}p - {size_str}", callback_data=cb_data)])
...
async def yt_format_callback(c: Client, cb: CallbackQuery):
    ...
    if not cb_data.startswith(f"yt_dl_{uid}_"):
        await cb.answer("You are not authorized for this action.", show_alert=True)
        return
    
    session_data = YT_SESSIONS.get(cb_data)
    if not session_data:
    ...
    fmt = session_data['format']
    title = session_data['title']
    
    asyncio.create_task(
        process_youtube_download(c, cb.message, uid, url, fmt, title)
    )
...
async def process_youtube_download(c, status_msg, uid, url, fmt, title):
    ...
    try:
        ...
        asyncio.create_task(
            sequential_upload_task(uid, c, status_msg.reply_to_message or status_msg, actual_path, renamed_file, status_msg.id, cancel_event)
        )
